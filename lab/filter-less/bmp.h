...
typedef uint8_t  BYTE;   // define a 1 byte  = 8 bits
typedef uint32_t DWORD;  // define a 4 bytes = 32 bits
typedef int32_t  LONG;   // define a 4 byes -/+ = 32 bits
typedef uint16_t WORD;   // define a 2 bytes = 16 bits
...
typedef struct
{
    WORD   bfType; // [type var "WORD"] name "bfType"
    DWORD  bfSize;
    WORD   bfReserved1;
    WORD   bfReserved2;
    DWORD  bfOffBits;
